...
if __name__ == "__main__":
    ...
    credited).
    """
    api_credentials = {'api_username': API_USERNAME, \
            'api_password': API_PASSWORD, 'api_signature': API_SIGNATURE, \
            'app_id': TEST_APP_ID}
    adaptpay = AdaptivePayment(**api_credentials)
    """ In the example below, the sender pays the receiver 10.00GBP. The
    following event sequence occurs:
        * You send a PayRequest.
        * You receive a response with a pay key.
        * You use the _ap-payment command to redirect Julie to PayPal to log
        in and approve the payment.
    """
    pay_request_kwargs = { \
            'receiver_email': 'mamadi_1262634965_per@gmail.com',
            'receiver_amount': 10.00, 'currency_code': 'GBP', \
    ...
    The response includes a pay key, which is a token that you use in
    subsequent calls to Adaptive Payments APIs to identify this particular
    payment.

    In this particular scenario, the payment was not preapproved. The response
    indicates that by returning the paymentExecStatus variable as CREATED
    instead of COMPLETED.
    """
    payresp = adaptpay.call(api_operation='Pay', api_request=payreqmsg)
    """ Redirecting Sender for Payment Approval
    Use the _ap-payment command redirect Julie to PayPal to log in and approve
    the payment:
