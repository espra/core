...
func (dec *Decoder) setFloat32(rv reflect.Value) (err error) {
...
func (dec *Decoder) setFloat64(rv reflect.Value) (err error) {
...
func (dec *Decoder) ReadFloat32() (value float32, err error) {
	data := make([]byte, 4)
	_, err = dec.r.Read(data)
	if err != nil {
		return
...
func (dec *Decoder) ReadFloat64() (value float64, err error) {
	data := make([]byte, 8)
	_, err = dec.r.Read(data)
	if err != nil {
		return
...
func (dec *Decoder) ReadUint64() (value uint64, err error) {
	bitShift := uint64(0)
	lowByte := 1
	data := make([]byte, 1)
	for lowByte > 0 {
