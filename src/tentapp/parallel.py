...
"""Support for parallel datastore queries on Google App Engine."""
...
from google.appengine.ext.db import Query, class_for_kind
    ...
    def rpc_callback(self, rpc):
        ...
        if more:
            ...
            if remaining and (remaining != self._prev):
                ...
                # logging.error("Requesting %r more for %r [%r]" % (remaining,
                #     self._value, len(buffer)
                # ))
...
# ------------------------------------------------------------------------------
# Parallel Query Object
# ------------------------------------------------------------------------------

class ParallelQuery(object):

    def __init__(
        self, model_class=Item, keys_only=False, query_key=None,
        ...
        try:
            ...
            if query_key:
                unset_keys = memcache.set_multi(
                    results, self.cache_duration, key_prefix, self.namespace
                    )
                if self.notify:
